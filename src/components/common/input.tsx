...
export const Input = (props: Props) => {
  const { labelText, icon: Icon, className, noMargin, isError, message, disabled, ...rest } = props;
  ...
  return (
    <div className={`mb-${noMargin ? '0' : '4'} relative${className ? ` ${className}` : ''}`}>
      ...
      <input
        {...rest}
        disabled={disabled}
        className={`w-full px-4 py-2 bg-gray-50 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-300 focus:border-orange-400 transition-all outline-none${hasIconClass}${isErrorClass}${disabledClass}`}
      />
